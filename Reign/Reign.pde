...
String[] t = {"Do you wanna stake him?","All hail the King?"};
int question = 0;

void setup() {
  size(640, 480);
  textSize(34);
  img[0] = loadImage("data/mads.png");
  img[0].resize(90, 150);
  img[1] = loadImage("data/dapw.jpg");
  img[1].resize(90, 150);
}
...
void scoreDisplay() {
  if(question < 2)
  text(t[question], width/2-225, height-100);
...
void imageDisplay() {
  if (question == 1)
    image(img[question], width/2-45, height/2-75);
  else if (question == 0) {
    image(img[question], width/2-45, height/2-75);
  } else {
